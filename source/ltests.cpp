...
#define checkobjref(g, f, t)                              \
    {                                                     \
        if (t)                                            \
            lua_longassert(testobjref(g, f, obj2gco(t))); \
    }

static void checkvalref(global_State* g, GCObject* f, const TValue* t)
{
    lua_assert(!iscollectable(t) || (righttt(t) && testobjref(g, f, gcvalue(t))));
}
...
static int lua_checkpc(lua_State* L, CallInfo* ci)
{
    if (!isLua(ci))
        ...
        return p->code <= ci->u.l.savedpc && ci->u.l.savedpc <= p->code + p->sizecode;
    }
}
...
static void checkgraylist(global_State* g, GCObject* o)
{
    ...
    while (o) {
        ...
        case LUA_TPROTO:
            ...
            break;
        default:
            lua_assert(0); /* other objects cannot be gray */
        }
    }
}
...
static int gc_color(lua_State* L)
{
    ...
    if (!iscollectable(o))
        ...
        lua_pushstring(L, isdead(G(L), obj) ? "dead" : iswhite(obj) ? "white"
                : isblack(obj)                                      ? "black"
                                                                    : "grey");
    }
    return 1;
