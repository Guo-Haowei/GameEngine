...
bool LoaderAssimp::load(Scene* data) {
    m_scene = data;
...
void LoaderAssimp::process_mesh(const aiMesh& mesh) {
    ...
    mesh_component.create_render_data();

    m_meshes.push_back(mesh_id);
}

ecs::Entity LoaderAssimp::process_node(const aiNode* node, ecs::Entity parent) {
