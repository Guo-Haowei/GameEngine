...
vec3 trace_cones(vec3 from, vec3 direction, float aperture) {
    float max_dist = 2.0 * c_worldSizeHalf;
    vec4 acc = vec4(0.0);
...
vec3 cone_diffuse(vec3 position, vec3 N) {
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        diffuse += g_diffuse_cones[i].weight * trace_cones(position, direction, aperture);
    }

    return diffuse;
}

vec3 cone_specular(vec3 position, vec3 direction, float roughness) {
    // TODO: brdf lookup
