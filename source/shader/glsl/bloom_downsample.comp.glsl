...
void main() {
    ...
    float x = input_tex_size.x;
    float y = input_tex_size.y;
    ...
    // textureSize(u_tmp_down_sample_input, 0);

    // ivec2 output_size = imageSize(u_output_image);

    // ivec2 output_uv = ivec2(int(tex_coord.x * output_size.x), int(tex_coord.y * output_size.y));
