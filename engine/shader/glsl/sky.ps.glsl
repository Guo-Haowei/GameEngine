...
void main() {
    ...
    vec3 day_extinction = exp(-exp(-((pass_pos.y + pass_fsun.y * 4.0) * (exp(-pass_pos.y * 16.0) + 0.1) / 80.0) / Br) * (exp(-pass_pos.y * 16.0) + 0.1) * Kr / Br) * exp(-pass_pos.y * exp(-pass_pos.y * 8.0) * 4.0) * exp(-pass_pos.y * 2.0) * 4.0;
    vec3 night_extinction = vec3(1.0 - exp(pass_fsun.y)) * 0.2;
