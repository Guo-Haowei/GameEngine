...
struct Vector<T, 2> : VectorBase<T, 2> {
    ...
    constexpr Vector(T p_x, T p_y)
        : x(p_x), y(p_y) {
    }

    template<Arithmetic U>
        requires(!std::is_same<T, U>::value)
    explicit constexpr Vector(U p_x, U p_y)
        : x(static_cast<T>(p_x)), y(static_cast<T>(p_y)) {
    }

    template<Arithmetic U>
        requires(!std::is_same<T, U>::value)
    explicit Vector(const Vector<U, 2>& p_rhs)
        : x(static_cast<T>(p_rhs.x)), y(static_cast<T>(p_rhs.y)) {
    }

    template<int N, int A, int B>
    constexpr Vector(const Swizzle2<T, N, A, B, -1, -1>& p_swizzle)
        : x(p_swizzle.d[A]), y(p_swizzle.d[B]) {
    }
