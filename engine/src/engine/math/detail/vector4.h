...
struct alignas(sizeof(T) * 4) Vector<T, 4> : VectorBase<T, 4> {
    ...
    explicit constexpr Vector(U p_x, U p_y, U p_z, U p_w)
        : x(static_cast<T>(p_x)), y(static_cast<T>(p_y)), z(static_cast<T>(p_z)), w(static_cast<T>(p_w)) {
    }

    template<Arithmetic U>
        requires(!std::is_same<T, U>::value)
    explicit Vector(const Vector<U, 4>& p_rhs)
        : x(static_cast<T>(p_rhs.x)), y(static_cast<T>(p_rhs.y)), z(static_cast<T>(p_rhs.z)), w(static_cast<T>(p_rhs.w)) {
    }

    explicit constexpr Vector(const Vector<T, 3>& p_vec, T p_w)
        : x(p_vec.x), y(p_vec.y), z(p_vec.z), w(p_w) {
    }

    explicit constexpr Vector(const Vector<T, 2>& p_vec1, const Vector<T, 2>& p_vec2)
        : x(p_vec1.x), y(p_vec1.y), z(p_vec2.x), w(p_vec2.y) {
    }

    explicit constexpr Vector(const Vector<T, 2>& p_vec, T p_z, T p_w)
        : x(p_vec.x), y(p_vec.y), z(p_z), w(p_w) {
    }

    template<int N, int A, int B, int C, int D>
    constexpr Vector(const Swizzle4<T, N, A, B, C, D>& p_swizzle)
        : x(p_swizzle.d[A]), y(p_swizzle.d[B]), z(p_swizzle.d[C]), w(p_swizzle.d[D]) {
    }
