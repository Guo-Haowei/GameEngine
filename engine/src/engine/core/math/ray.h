...
public:
    ...
    Vector3f Direction() const { return glm::normalize(m_end - m_start); }

    bool Intersects(const AABB& p_aabb) { return TestIntersection::RayAabb(p_aabb, *this); }
    bool Intersects(const Vector3f& p_a, const Vector3f& p_b, const Vector3f& p_c) { return TestIntersection::RayTriangle(p_a, p_b, p_c, *this); }
