...
class Ray {
public:
    Ray(const NewVector3f& p_start, const NewVector3f& p_end) : m_start(p_start), m_end(p_end), m_dist(1.0f) {}
    ...
    bool Intersects(const NewVector3f& p_a,
                    const NewVector3f& p_b,
                    const NewVector3f& p_c) {
...
private:
    const NewVector3f m_start;
    const NewVector3f m_end;
